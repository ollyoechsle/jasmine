...
public class JasmineGPSubObject extends JasmineGP {
    ...
    public JasmineGPSubObject(JasmineProject p, int drsType) {
        super(drsType, p.getObjectClasses().size());
        this.p = p;
        trainingData = JasmineUtils.getLabelledSubObjects(p);
    }
    ...
    public void initialise(Evolve e, GPParams params) {
        ...
        Vector<Terminal> set = JasmineUtils.getTerminalSet(getStandardTerminals(null), terminalMetaData);
    ...
    */
    private PCM buildProgramClassificationMap(DataStack data, Individual ind) {
        ...
        for (int i = 0; i < trainingData.size(); i++) {
            ...
            // get the data
            ExtraShapeData d = trainingData.elementAt(i);
    ...
    public ClassResults describe(GPActionListener gpActionListener, Individual ind, DataStack data, int index) {
        ...
        for (int i = 0; i < trainingData.size(); i++) {
            ...
            if (results.getClassResult(classID) == null) {
                ...
                String className = p.getSubObjectClass(classID).name ;
